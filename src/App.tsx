...
import { addOfficerUpdate, createTicket, getTicket, verifyOfficerUpdate } from './services/api'
...
export default function App() {
  const [view, setView] = useState<'report' | 'success' | 'track' | 'officer'>('report')
  ...
  return <main>
    <header className="topbar"><button className="brand" onClick={() => setView('report')}><span className="brand-mark"><ShieldCheck size={21}/></span><span>Smart <b>CPGRAMS</b><small>Citizen civic services</small></span></button><div className="header-actions"><button className="officer-link" onClick={() => setView('officer')}><UserRound size={16}/> Officer demo</button><button className="track-link" onClick={() => setView('track')}><Search size={17}/> Track complaint</button></div></header>
