import { LocateFixed, MapPin, Mic, Navigation, Search, Sparkles } from 'lucide-react'
...
const civicTerms = ['accident', 'blocked', 'drain', 'footpath', 'garbage', 'leak', 'pipeline', 'pothole', 'public', 'road', 'sanitation', 'streetlight', 'waste', 'water']

function polishTranscript(value) {
  return value.trim()
    .replace(/\bpot\s+hole\b/gi, 'pothole')
    .replace(/\bstreet\s+light\b/gi, 'streetlight')
    .replace(/\bfoot\s+path\b/gi, 'footpath')
    .replace(/\bpipe\s+line\b/gi, 'pipeline')
    .replace(/\s{2,}/g, ' ')
}

function bestTranscript(result) {
  return Array.from(result).map(alternative => {
    const transcript = polishTranscript(alternative.transcript)
    const contextMatches = civicTerms.filter(term => new RegExp(`\\b${term}`, 'i').test(transcript)).length
    return { transcript, score: (alternative.confidence || 0) + contextMatches * 0.04 }
  }).sort((first, second) => second.score - first.score)[0]?.transcript || ''
}

export default function NewComplaint() {
  const [step, setStep] = useState(1); const [category, setCategory] = useState(''); const [complaint, setComplaint] = useState('')
  const [listening, setListening] = useState(false); const [interimTranscript, setInterimTranscript] = useState(''); const recognitionRef = useRef(null)
  const [point, setPoint] = useState(null); const [confirmed, setConfirmed] = useState(null); const [path, setPath] = useState('current'); const [geoStatus, setGeoStatus] = useState('requesting')
  const [message, setMessage] = useState('We are requesting permission to find your location. You can also search for a different place.'); const [query, setQuery] = useState(''); const [manualLabel, setManualLabel] = useState(''); const [manualLatitude, setManualLatitude] = useState(''); const [manualLongitude, setManualLongitude] = useState(''); const [results, setResults] = useState([]); const [searching, setSearching] = useState(false); const [submitting, setSubmitting] = useState(false)
  ...
  function speak() {
    ...
    if (!Recognition) { setMessage('Voice input is not supported in this browser. You can type your answer instead.'); return }
    const recognition = new Recognition(); recognitionRef.current = recognition; recognition.lang = 'en-IN'; recognition.interimResults = true; recognition.continuous = true; recognition.maxAlternatives = 5
    setInterimTranscript(''); setListening(true); setMessage('Listening continuously... speak naturally. Tap the microphone again when you are finished.')
    recognition.onresult = event => { let interim = ''; for (let index = event.resultIndex; index < event.results.length; index += 1) { const result = event.results[index]; const transcript = bestTranscript(result); if (!transcript) continue; if (result.isFinal) setComplaint(current => `${current}${current ? ' ' : ''}${transcript}`); else interim += `${interim ? ' ' : ''}${transcript}` } setInterimTranscript(interim) }
    recognition.onerror = event => { setListening(false); setInterimTranscript(''); recognitionRef.current = null; setMessage(event.error === 'not-allowed' ? 'Microphone permission was denied. Allow microphone access or type your answer instead.' : 'Voice input could not be started. You can type your answer instead.') }
    recognition.onend = () => { setListening(false); setInterimTranscript(''); recognitionRef.current = null; setMessage('Voice input finished. Review the text before continuing.') }
    try { recognition.start() } catch { setListening(false); recognitionRef.current = null; setMessage('Voice input is already starting. Try again in a moment.') }
  }
  ...
  return <main className="complaint-page">
    <header className="complaint-header"><p className="eyebrow"><Sparkles size={15} /> New complaint</p><h1>Tell us what needs fixing.</h1><p>We will guide your report to the right local authority and keep you involved until it is resolved.</p></header><Stepper steps={steps} currentStep={step} />
